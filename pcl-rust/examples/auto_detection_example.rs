...
use pcl::io::{detect_format, detect_format_from_ext, detect_format_from_file_content, load_xyz, FileFormat};
use pcl::common::PointCloudXYZBuilder;
use pcl::io::{PcdIoXYZ, PlyIoXYZ, BinaryFormat};
...
fn main() -> Result<()> {
    ...
    for (filename, expected_format) in &test_files {
        ...
        match detect_format_from_ext(filename) {
            Ok(format) => {
                ...
                if filename.contains('.') {
                    assert_eq!(format, *expected_format, "Extension detection failed for {}", filename);
                }
            }
            Err(e) => {
                if filename.contains('.') {
                    ...
                    println!("  Extension detection: Failed (expected for no extension): {}", e);
                }
            }
        }
        ...
        match detect_format_from_file_content(filename) {
            Ok(format) => {
                ...
                assert_eq!(format, *expected_format, "Content detection failed for {}", filename);
            }
        ...
        match detect_format(filename) {
            Ok(format) => {
                ...
                assert_eq!(format, *expected_format, "Auto detection failed for {}", filename);
            }
    ...
    for (filename, _) in &test_files {
        ...
        match load_xyz(filename) {
            Ok(loaded_cloud) => {
                ...
                assert_eq!(loaded_cloud.size(), sample_cloud.size(), 
                          "Loaded cloud size mismatch for {}", filename);
                
                // Basic validation that points were loaded correctly
                assert!(!loaded_cloud.empty(), "Loaded cloud is empty for {}", filename);
            }
    ...
    std::fs::write("corrupted.pcd", "# .PCD v0.7\nVERSION 0.7\nFIELDS x y z\nSIZE 4 4 4\nTYPE F F F\nCOUNT 1 1 1\nWIDTH 1\nHEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\nPOINTS 1\nDATA ascii\n// Missing data")?;
    ...
    let cleanup_files = ["test_cloud.pcd", "test_cloud.ply", "test_cloud_no_ext", "test.txt", "corrupted.pcd"];

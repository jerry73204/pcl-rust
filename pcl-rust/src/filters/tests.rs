...
mod general_filter_tests {
    use super::*;
    use crate::filters::{PassThrough, VoxelGrid, StatisticalOutlierRemoval, RadiusOutlierRemoval};

    #[test]
    fn test_pass_through_creation() -> PclResult<()> {
        // Test creating PassThrough filter for different point types
        let _filter_xyz: PassThrough<PointXYZ> = PassThrough::new()?;
        let _filter_xyzrgb: PassThrough<PointXYZRGB> = PassThrough::new()?;
        Ok(())
    }
    ...
    fn test_pass_through_field_filter() -> PclResult<()> {
        ...
        let mut filter = PassThrough::<PointXYZ>::new()?;
        let mut cloud = PointCloud::<PointXYZ>::new()?;
        
        // Create test data with varied Z values
        for i in 0..10 {
            cloud.push(i as f32, 0.0, i as f32 * 0.5)?;
        }
    ...
    fn test_voxel_grid_creation() -> PclResult<()> {
        ...
        let _filter_xyz: VoxelGrid<PointXYZ> = VoxelGrid::new()?;
        let _filter_xyzrgb: VoxelGrid<PointXYZRGB> = VoxelGrid::new()?;
        Ok(())
    }
    ...
    fn test_voxel_grid_downsampling() -> PclResult<()> {
        ...
        for x in 0..10 {
            for y in 0..10 {
                for z in 0..10 {
                    cloud.push(x as f32 * 0.1, y as f32 * 0.1, z as f32 * 0.1)?;
                }
            }
        }
    ...
    fn test_voxel_grid_uniform_leaf() -> PclResult<()> {
        ...
        for i in 0..50 {
            cloud.push(i as f32 * 0.1, i as f32 * 0.1, i as f32 * 0.1)?;
        }
    ...
    fn test_statistical_outlier_removal_creation() -> PclResult<()> {
        ...
        let _filter: StatisticalOutlierRemoval<PointXYZ> = StatisticalOutlierRemoval::new()?;
        Ok(())
    }
    ...
    fn test_statistical_outlier_removal_basic() -> PclResult<()> {
        ...
        let mut filter = StatisticalOutlierRemoval::<PointXYZ>::new()?;
        let mut cloud = PointCloud::<PointXYZ>::new()?;
        
        // Create mostly uniform data with some outliers
        for i in 0..50 {
            cloud.push(i as f32 * 0.1, 0.0, 0.0)?;
        }
        // Add outliers
        cloud.push(5.0, 10.0, 10.0)?;
        cloud.push(-5.0, -10.0, -10.0)?;
    ...
    fn test_statistical_outlier_negative() -> PclResult<()> {
        ...
        cloud.push(25.0, 0.0, 0.0)?; // Outlier
    ...
    fn test_radius_outlier_removal_creation() -> PclResult<()> {
        ...
        let _filter: RadiusOutlierRemoval<PointXYZ> = RadiusOutlierRemoval::new()?;
        Ok(())
    }
    ...
    fn test_radius_outlier_removal_basic() -> PclResult<()> {
        ...
        let mut filter = RadiusOutlierRemoval::<PointXYZ>::new()?;
        let mut cloud = PointCloud::<PointXYZ>::new()?;
    ...
    fn test_filter_xyzrgb() -> PclResult<()> {
        ...
        let mut voxel = VoxelGrid::<PointXYZRGB>::new()?;
...
mod filter_integration_tests {
    ...
    fn test_filter_pipeline() -> PclResult<()> {
        ...
        // Add outliers
        cloud.push(50.0, 50.0, 50.0)?;
        cloud.push(-50.0, -50.0, -50.0)?;
        
        // Step 1: Remove outliers
        let mut outlier_filter = StatisticalOutlierRemoval::<PointXYZ>::new()?;
        ...
        let mut pass_filter = PassThrough::<PointXYZ>::new()?;
    ...
    fn test_filter_empty_result() -> PclResult<()> {
        ...
        let mut voxel = VoxelGrid::<PointXYZ>::new()?;
        voxel.set_input_cloud(&cloud)?;
        ...
        let mut pass = PassThrough::<PointXYZ>::new()?;
...
mod filter_performance_tests {
    ...
    fn test_filter_edge_cases() -> PclResult<()> {
        ...
        cloud.push(0.0, f32::MAX / 2.0, 0.0)?;
        cloud.push(0.0, f32::MIN / 2.0, 0.0)?;
...
mod filter_builder_tests {
    use super::*;
    use crate::filters::{VoxelGridBuilder, PassThroughBuilder};
    ...
    fn test_voxel_grid_builder() -> PclResult<()> {
        ...
        let filtered = VoxelGridBuilder::new()
    ...
    fn test_pass_through_builder() -> PclResult<()> {
        ...
        for i in 0..10 {
            cloud.push(0.0, 0.0, i as f32)?;
        }
        
        let filtered = PassThroughBuilder::new()
            .field_name("z")?
            .limits(2.0, 7.0)?
            .filter(&cloud)?;

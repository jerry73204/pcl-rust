...
std::unique_ptr<pcl::MinCutSegmentation<pcl::PointXYZ>>
new_min_cut_segmentation_xyz() {
  try {
    return std::make_unique<pcl::MinCutSegmentation<pcl::PointXYZ>>();
  } catch (const std::exception &e) {
    return nullptr;
  }
}

void set_input_cloud_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc,
                                 const pcl::PointCloud<pcl::PointXYZ> &cloud) {
...
void set_foreground_points_min_cut_xyz(
    ...
    const pcl::PointCloud<pcl::PointXYZ> &foreground_points) {
  mc.setForegroundPoints(foreground_points.makeShared());
}

void set_sigma_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc,
                           double sigma) {
  mc.setSigma(sigma);
}

double get_sigma_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc) {
  return mc.getSigma();
}

void set_radius_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc,
                            double radius) {
  mc.setRadius(radius);
}

double get_radius_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc) {
  return mc.getRadius();
}

void set_number_of_neighbours_min_cut_xyz(
    pcl::MinCutSegmentation<pcl::PointXYZ> &mc, int32_t k) {
  mc.setNumberOfNeighbours(k);
}

int32_t get_number_of_neighbours_min_cut_xyz(
    pcl::MinCutSegmentation<pcl::PointXYZ> &mc) {
  return mc.getNumberOfNeighbours();
}

void set_source_weight_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc,
                                   double weight) {
  mc.setSourceWeight(weight);
}

double
get_source_weight_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc) {
  return mc.getSourceWeight();
}

rust::Vec<int32_t>
extract_min_cut_xyz(pcl::MinCutSegmentation<pcl::PointXYZ> &mc) {
  try {
...
std::unique_ptr<pcl::ExtractPolygonalPrismData<pcl::PointXYZ>>
new_extract_polygonal_prism_xyz() {
  try {
    return std::make_unique<pcl::ExtractPolygonalPrismData<pcl::PointXYZ>>();
  } catch (const std::exception &e) {
    return nullptr;
  }
}
...
void set_input_planar_hull_polygonal_prism_xyz(
    pcl::ExtractPolygonalPrismData<pcl::PointXYZ> &prism,
    const pcl::PointCloud<pcl::PointXYZ> &hull) {
  prism.setInputPlanarHull(hull.makeShared());
}

void set_height_limits_polygonal_prism_xyz(
    pcl::ExtractPolygonalPrismData<pcl::PointXYZ> &prism, double height_min,
    double height_max) {
  prism.setHeightLimits(height_min, height_max);
}

rust::Vec<int32_t> segment_polygonal_prism_xyz(
    pcl::ExtractPolygonalPrismData<pcl::PointXYZ> &prism) {
  try {
...
std::unique_ptr<pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ>>
...
int32_t get_max_window_size_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf) {
  return pmf.getMaxWindowSize();
}

void set_slope_pmf_xyz(pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf,
                       float slope) {
  pmf.setSlope(slope);
}
...
float get_initial_distance_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf) {
  return pmf.getInitialDistance();
}

void set_cell_size_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf, float cell_size) {
  pmf.setCellSize(cell_size);
}

float get_cell_size_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf) {
  return pmf.getCellSize();
}

void set_base_pmf_xyz(pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf,
                      float base) {
  pmf.setBase(base);
}
...
void set_exponential_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf, bool exponential) {
  pmf.setExponential(exponential);
}

bool get_exponential_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf) {
  return pmf.getExponential();
}

rust::Vec<int32_t> extract_ground_pmf_xyz(
    pcl::ProgressiveMorphologicalFilter<pcl::PointXYZ> &pmf) {
  try {
...
std::unique_ptr<pcl::ConditionalEuclideanClustering<pcl::PointXYZ>>
new_conditional_euclidean_clustering_xyz() {
  try {
    return std::make_unique<
        pcl::ConditionalEuclideanClustering<pcl::PointXYZ>>();
  } catch (const std::exception &e) {
    return nullptr;
  }
}
...
void set_cluster_tolerance_conditional_euclidean_xyz(
    pcl::ConditionalEuclideanClustering<pcl::PointXYZ> &cec, float tolerance) {
  cec.setClusterTolerance(tolerance);
}

float get_cluster_tolerance_conditional_euclidean_xyz(
    pcl::ConditionalEuclideanClustering<pcl::PointXYZ> &cec) {
  return cec.getClusterTolerance();
}

void set_min_cluster_size_conditional_euclidean_xyz(
    pcl::ConditionalEuclideanClustering<pcl::PointXYZ> &cec, int32_t min_size) {
...
void set_max_cluster_size_conditional_euclidean_xyz(
    pcl::ConditionalEuclideanClustering<pcl::PointXYZ> &cec, int32_t max_size) {
